...
NEW_SYSTEM_DATE = "2026-04-26T12:00:00Z"  # v8.3: Profitability overhaul — tighter SL/TP, trailing stops, regime detection
...
def fetch_signals(limit: int = 100):
    ...
    try:
        ...
        if not client:
            return []
        # No gte filter: table is bounded by 7-day retention + LIMIT walks
        # idx_signals_timestamp DESC cleanly. The filter caused intermittent
        # statement timeouts after a large DELETE made planner stats stale.
...
def main():
    ...
    st.caption(f"Last updated: {kst_now.strftime('%Y-%m-%d %H:%M:%S KST')} | v8.3 started: Apr 26, 2026 | v8.3 (tighter SL/TP + trailing stops + regime detection, 5 strategies, 6 coins) | dash hotfix: drop unused reasoning col")
